...
def timecode_to_seconds(timecode):
    """Convertit un timecode HH:MM:SS.ms en secondes."""
    hh, mm, ss_ms = timecode.split(':')
    ss, ms = ss_ms.split('.')
    return int(hh) * 3600 + int(mm) * 60 + int(ss) + int(ms) / 1000

def seconds_to_timecode(seconds):
    """Convertit un nombre de secondes en timecode HH:MM:SS.ms."""
...
def process_kdenlive_file(file_path):
    """Extrait les timecodes des clips en tenant compte du décalage cumulé dans chaque playlist."""
