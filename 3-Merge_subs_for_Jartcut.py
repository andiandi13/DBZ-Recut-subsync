...
def extract_number_from_filename(filename):
    # Chercher les séquences numériques dans le nom de fichier, peu importe les zéros (ex. 001, 01, 1)
    match = re.search(r'(\d+)\.ass', filename)  # Recherche de tout numéro suivi de '.ass'
    if match:
        return match.group(1).zfill(3)  # Retourner le numéro formaté sur 3 chiffres
    return None
...
def merge_files(merge_target, files_to_merge):
    # Créer la commande pour exécuter subdigest.py avec les fichiers à fusionner
    command = ['python', 'subdigest.py', '-o', merge_target]  # Option -o pour la sortie
    for file in files_to_merge:
        command += ['-i', file]  # Ajouter l'option -i pour chaque fichier d'entrée

    try:
        subprocess.run(command, check=True)
        print(f"Fusion réussie pour {merge_target}")
    except subprocess.CalledProcessError:
        print(f"Erreur lors de la fusion de {merge_target}")

# Fonction principale
def main():
    # Collecte tous les fichiers .ass
    ass_files = collect_ass_files(input_directory)

    # Pour chaque entrée dans le plan de fusion
    for target_file, files_to_merge in merging_plan.items():
        files_found = []

        # Parcourir les numéros de fichiers à fusionner et chercher les fichiers correspondants
        for file_number in files_to_merge:
            # Rechercher tous les fichiers qui contiennent le numéro, qu'il soit sous forme de '001', '01' ou '1'
            for file_name, file_number_in_ass in ass_files:
                # Vérifier si le numéro est contenu dans le nom du fichier
                if file_number in file_name:  # Contient le numéro dans le nom (et non une correspondance exacte)
                    files_found.append(os.path.join(input_directory, file_name))  # Chemin complet du fichier
